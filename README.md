...
**Directory Structure:**
Explainations of the SKTime-based models can be found in Modeling_with_SKTime.ipynb, the simple linear regression model in Modeling_with_LR.ipynb, and the LSTM in Modeling_with_LSTM.ipynb. Notebooks with the prefixes 01 through 06 provide a walkthrough of our data cleaning, aggregating, and engineering process. The file utils.py contains functins provided by the ThinkOnward Challenge, as well as those written for our analysis. The Results directory provides certain visualizations, data, and performance metrics relating to our results. The Old_Documents directory serves as storage for old, unused code and documentation which is not currently part of the project but which we choose to keep for reference.
