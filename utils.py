...
def combine_agg_ts(state,
    ...
    #print('event_types_state:',event_types_state)
...
def chunk_aggregated_engineered_data(df_full, fips, outpath, filename, interval_X, interval_y):
    """
    Chunk dataset into training samples of several days and save to pickle, using the 6hr aggregated datafile (eaglei_noaa_era5_full.parquet)

    Parameters:
    intervals_X (int): number of 6hr intervals in the chunk to be used as predictors
    days_per_y (int): number of days in the chunk to be used as target
    df (pandas dataframe): dataframe of aggregated 6-hourly weather event data
    fips (int): county code
    ...
    time_pred_start (list): list of datetime objects for prediction start times
    Raises:
    FileNotFoundError: if the output directory doesn't exist
    """
    
    # get county level data 
    df = df_full[df_full['fips_code'] == fips].drop(columns=['NAME', 'datetime',
                                                    'STUSPS', 'neighbors', 'Subregion',
                                                    'centroid_rounded']) # drop non-numeric columns
    timeseries = df_full[df_full['fips_code'] == fips]['datetime']

    intervals_per_chunk = interval_X + interval_y

    # create chunked data
    n_chunks = int(len(df)/intervals_per_chunk)
    chunked_X = np.zeros([interval_X, len(df.columns) - 1, n_chunks])*np.nan
    chunked_y = np.zeros([intervals_per_chunk - interval_X, 1, n_chunks])*np.nan
    datetimes_pred_start = []
    for i in range(n_chunks):
        chunk = df.iloc[i*intervals_per_chunk:(i+1)*intervals_per_chunk,:]
        datetimes_pred_start += [timeseries.iloc[i*intervals_per_chunk + interval_X]]
        X = chunk.iloc[:interval_X,:].drop(columns=['customers_out']) # drop target
        y = chunk.iloc[interval_X:,:]['customers_out'] / chunk.iloc[interval_X:,:]['POPULATION'] # fraction of population without power
        chunked_X[:,:,i] = X.reset_index(drop=True)
        chunked_y[:,:,i] = np.array(y).reshape(-1,1)
    
    # save chunked data
    if not os.path.isdir(outpath):
        raise FileNotFoundError(outpath + ' does not exist.')
    out_file = open(outpath + '/X_chunked_agg_' + str(fips) + '_' + filename + '.pkl', 'ab')
    pickle.dump(chunked_X, out_file)
    out_file.close()
    out_file = open(outpath + '/y_chunked_agg_' + str(fips) + '_' + filename + '.pkl', 'ab')
    pickle.dump(chunked_y, out_file)
    out_file.close()
    
    return chunked_X, chunked_y, datetimes_pred_start
